...
import { pickSeatsUpperCenterAdjacency, SeatCell } from './seatEngine.js'
...
export async function checkAndSelect(config: AppConfig) {
             ...
             await page.evaluate(({rowLabel, col}) => {
                 ...
                 for (const r of rows) {
                     if (r.querySelector('[class*="rowLabel"]')?.textContent?.includes(rowLabel)) {
                         ...
                         // 1-based index
                         if (seats[col-1]) (seats[col-1] as HTMLElement).click()
                     }
                 }
...
async function extractSeatGrid(page: Page): Promise<SeatCell[][]> {
...
function simulateSeats(): SeatCell[][] {
  const rows: SeatCell[][] = []
